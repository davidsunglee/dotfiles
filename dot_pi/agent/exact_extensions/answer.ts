...
/**
 * Prefer GPT-5.4 for extraction when available, otherwise fallback to haiku or the current model.
 */
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Select the best model for extraction (prefer GPT-5.4, then haiku)
